...
namespace WineShop.Controllers
{
    public class PostController : Controller
    {
        ...
        [HttpPost]
        public ActionResult UpdateCustomer(WineShop.Models.Customer customer)
        {
            if (Tools.Security.Customer.GetCurrent() == null) { return RedirectToAction("Index", "Home"); }
            else
            {
                Datas.Ecommerce.DataSetCustomerTableAdapters.tbCustomerTableAdapter tbCustomerTableAdapter = new Datas.Ecommerce.DataSetCustomerTableAdapters.tbCustomerTableAdapter() { Connection = Tools.Security.Connection.odbcConnection };
                try
                {
                    if (tbCustomerTableAdapter.UpdateInformation(
                        ...
                        , Tools.Security.Customer.GetCurrent().Uid).Equals(1)) { Tools.Security.Customer.ReloadCustomer(); }
        ...
        public ActionResult CheckoutReview(string Cmd)
        {
            ...
            try
            {
                if (tbCustomerCommandTableAdapter.Insert(
                    ...
                    , Tools.App.Currency.GetCurrent().Uid
                    , Tools.App.Currency.GetCurrent().Symbol
                    , Tools.App.Currency.GetCurrent().Description
                    , Tools.App.Currency.GetCurrent().Rate
